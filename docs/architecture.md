...
| 审查 Prompt | `lib/amazon/listing-audit-prompt.ts` | 2026 标题 75 字符、Item Highlights、适老品类 |
| 适老清单 | `lib/amazon/mobility-checklist.ts` | 属性字段与同义词参考 |
| API | `POST /api/amazon/audit` | `{ asin \| manual }` → snapshot + audit |
| UI | `app/amazon/page.tsx` | 审查报告、复制建议、手动降级 |
...
- 批量目录（CLI 侧）处理
- Amazon 审查 → Sanity 同步、改写稿一键应用、SP-API 直连
